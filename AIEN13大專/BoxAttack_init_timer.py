...
class Object:
    ...
    def insert_object(self, myRectangle):
        roi = myRectangle[self.init_y: self.init_y +
                          self.size, self.init_x: self.init_x + self.size]
...
with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
    while True:
        ...
        h, w, c = frame.shape
        x1, y1, x2, y2 = obj.init_x, obj.init_y, (
            obj.init_x + obj.size), (obj.init_y + obj.size)
        frame = cv2.cvtColor(cv2.flip(frame, 1), cv2.COLOR_BGR2RGB)
        results = holistic.process(frame)
        obj.insert_object(frame)
        game_time = 8+init_time
        second = (game_time+2) - float(time.time() - time_it)
        if second > 0 and init_done == 1:
            game_over = 0
            cv2.putText(frame, "Game Start", (10, 50),
                        cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
            m, s = divmod(int(second), 60)
            time_left = str(m).zfill(2) + ":" + str(s).zfill(2)
            cv2.putText(frame, time_left, (500, 20),
                        cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
        elif second <= 0:
            cv2.putText(frame, "Time's up!", (130, 250),
        ...
        if results.right_hand_landmarks:
            ...
            if x_min > x1 and y_min > y1 and x_max < x2 and y_max < y2:
                ...
                if second > 0 and init_done == 0:
                    ...
                    cv2.putText(frame, time_left, (10, 50),
                                cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
                    ...
                    cv2.putText(frame, "Restart?", (170, 300),
                                cv2.FONT_HERSHEY_PLAIN, 4, (0, 255, 0), 2)
                else:
                    # event.wait(0.3)
                    init_done = 1
                ...
                time_it = time.time()
                cv2.putText(frame, "put your hand in the frame",
                            (10, 50), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
            else:
                pass
